...
- Calculate the total balance and display it on the page
- The total balance should be color coded dependending on if it's positive or negative
- The total balance should be updated each time a new expense or income is added
- Each created budget item should have a delete button next to it
- There should be a category dropdown when creating budget items
- The information should be saved in local storage
- The budget categories should be stored in a JSON file that is loaded

The project should also have a linter installed and configured, like ESLint, Prettier, Biome or Oxlint. The dependency should be in the devDependencies part of the `package.json` file.

## What Copilot should evaluate
### Project structure
- No compiled `.js` files committed unless required
...
|------------------|-----------------------------------------------------------------------------------------|
| Add income       | Function or class method that stores an entry with positive amount                      |
| Add expense      | Function or class method that stores an entry with negative amount or marked as expense |
| Track entries    | Array or similar structure storing all entries                                          |
| Calculate totals | Correct summation of all entries                                                        |
| Display summary  | DOM rendering showing entries and totals                                                |
